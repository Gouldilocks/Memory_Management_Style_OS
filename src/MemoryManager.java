...
class MemoryManager {
  ...
  public int findMemoryWorstFit(int size) {
    ...
    openBlocks = HelperFunctions.findLargeEnoughBlock(openBlocks, size);
    ...
    // Sort by size
    openBlocks = HelperFunctions.sortBySize(openBlocks);
  ...
  public void reportMemory() {
    ...
    for (Memory memory : allocatedMemory) {
      ...
      System.out
          .println("Addresses: [" + (beginMemory) + "," + (endMemory - 1) + "]" + " Process " + memory.getProcess());
